...
from gazebo_msgs.srv import SetModelState, GetModelState, SetPhysicsProperties, SetPhysicsPropertiesRequest
...
class SubtCaveForwardEnv(gym.Env):
    def __init__(self):
    ...
    def get_observation(self):
        ...
        self.total_dis += np.linalg.norm(diff)
        self.last_pos = new_pos

        # state: laserscan + relative pos from last frame
        state = self.scan_once()
        state = np.append(state, diff[:2])
