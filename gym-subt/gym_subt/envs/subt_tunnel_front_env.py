...
class SubtTunnelFrontEnv(gym.Env):
    ...
    def step(self, action):
        ...
        cmd_vel = Twist()
        cmd_vel.linear.x = self.scale_linear(action[0], self.action_bound['linear'])
        cmd_vel.angular.z = self.scale_angular(action[1], self.action_bound['angular'])
        ...
        reward *= 10 if (action[0] > 0) else 1
        reward *= action[0]
